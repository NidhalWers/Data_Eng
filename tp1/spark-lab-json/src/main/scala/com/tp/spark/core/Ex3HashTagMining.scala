...
object Ex3HashTagMining {
  ...
  def hashtagMentionedOnTweet(): RDD[String] = {
    loadData().filter(tweet => tweet.text.contains("#"))
      .flatMap( tweet => tweet.text.split(" ")
                                  .filter(word => word.startsWith("#") && word.size>1)
  ...
  def countMentions(): RDD[(String, Int)] = {
    hashtagMentionedOnTweet().map(hashtag => (hashtag,1))
      .reduceByKey(_+_)
  }
  ...
  def top10HashTags(): Array[(String, Int)] = {
    ListMap(countMentions().collect().toSeq.sortWith(_._2 > _._2):_*).take(10).toArray
  }
